...
function writeChildRuntimeConfig() {
  const childConfig = {
    ...
    relationshipSearchTargetMaxScrollRounds: Math.max(1, Number(runtimeConfig.relationshipSearchTargetMaxScrollRounds || 60)),
    relationshipSearchWaitMs: Math.max(250, Number(runtimeConfig.relationshipSearchWaitMs || 900)),
    publicConnectionRetryDelayMs: normalizeNumberAtLeast(runtimeConfig.publicConnectionRetryDelayMs, 6000, 1500),
    publicConnectionRetryMaxDelayMs: normalizeNumberAtLeast(runtimeConfig.publicConnectionRetryMaxDelayMs, 30000, 2000),
    publicConnectionCandidateMaxAttempts: normalizeIntegerInRange(runtimeConfig.publicConnectionCandidateMaxAttempts, 3, 1, 10),
    ...
    publicConnectionDialogMissingMaxAttempts: normalizeIntegerInRange(runtimeConfig.publicConnectionDialogMissingMaxAttempts, 2, 1, 10),
    publicConnectionRateLimitAccountSwitchEnabled: runtimeConfig.publicConnectionRateLimitAccountSwitchEnabled !== false,
    publicConnectionMaxScraperProfileSwitches: normalizeIntegerInRange(runtimeConfig.publicConnectionMaxScraperProfileSwitches, 3, 0, 10),
    relationshipSearchInputMaxAttempts: normalizeIntegerInRange(runtimeConfig.relationshipSearchInputMaxAttempts, 3, 1, 10),
    ...
    navigationTimeoutMs: Math.max(30000, Number(runtimeConfig.navigationTimeoutMs || 30000)),
    postLoginWaitMs: Math.max(500, Number(runtimeConfig.postLoginWaitMs || 2500)),
    skipDebugArtifacts: runtimeConfig.skipDebugArtifacts === true,
    blockHeavyResources: runtimeConfig.blockHeavyResources === true,
