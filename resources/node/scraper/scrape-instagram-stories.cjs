...
async function runInstagramStoriesScanFlow(context) {
  const {
    page,
    username,
    profileUrl,
  ...
  scanState.gracefullyStopped = Boolean(markGracefulStopIfRequested('stories', {
    loaded: 0,
    expectedCount: 0,
  }));

  if (scanState.gracefullyStopped) {
    notes.push('Instagram-Story-Scan wurde ueber die Oberflaeche beendet.');
...
if (require.main === module) {
  runInstagramScraperModeWrapper({
    defaultMode: 'stories',
    allowedModes: ['stories', 'story-scan'],
  });
}

module.exports = {
  runInstagramStoriesScanFlow,
};
